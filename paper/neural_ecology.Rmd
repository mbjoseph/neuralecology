...
output: 
  bookdown::pdf_document2:
    keep_tex: TRUE
    toc: FALSE
    # uncomment below for submission (and change title to "")
    # includes:
    #   before_body: title.sty
    #   in_header: doc-prefix.tex
    # uncomment below for preprint (and include title in this Rmd file)
    extra_dependencies:
...
This paper describes an emergent class of hierarchical models parameterized by neural networks --- neural hierarchical models --- and develops a case study with bird populations in the United States.
The derivation of such models is outlined beginning with the relationship between linear regression and neural networks, and analogously between a hierarchical ecological model (a dynamic occupancy model), and its neural counterpart. 
The resulting neural hierarchical model, trained on millions of detection/non-detection time series for hundreds of species, provides insights into colonization and extinction at a continental scale. 
The model also demonstrates good quantitative predictive performance and reasonable qualitative results about range shifts and macroecological patterns of population persistence at range boundaries.
...
While ecologists are beginning to use such approaches, e.g., to identify animals in camera trap images [@norouzzadeh2018automatically], there has been relatively little integration of deep neural networks in ecological models of population dynamics.
...
where $w$ is a parameter vector (Fig. \@ref(fig:figure2)a). 
Predicted values are linear combinations of the inputs $x$ due to the product $w^T x$, which restricts the complexity of the mapping from $x$ to $y$. 
...
The activations are passed to a differentiable nonlinear activation function $g$ to obtain the "hidden units" of the first layer $h^{(1)} = g(a^{(1)})$ (Fig. \@ref(fig:figure2)b).

The final layer of a neural network maps the last hidden layer to an output. 
...
The output activation can be written as the composition of functions that transform the inputs $x$:
...
"Deep" neural networks with many sequential hidden layers also possess desirable theoretical properties for function representation [@lu2017expressive].
...
```{r figure2, fig.cap="Computation graphs for (a) linear regression, (b) a neural network with one hidden layer, (c) a dynamic occupancy model, (d) a single species neural dynamic occupancy model, and (e) a deep multi-species neural dynamic occupancy model with species embeddings. Yellow and red indicate quantities specific to process and observation components respectively. Inputs are represented by $x$, and predicted values in panels (a) and (b) by $y$. Hidden layers are represented by $h$, with layer-specific superscripts. Outputs include initial occupancy ($\\psi_1$), persistence ($\\phi$), colonization ($\\gamma$), and detection ($p$) probabilities. Latent species embedding vectors are represented by $v$, and GRU indicates a gated recurrent unit.", fig.show='hold', fig.width = 2}
knitr::include_graphics(here('fig', 'fig2.pdf'))
```
...
One can propose neural variants of occupancy models [@mackenzie2002estimating], dynamic occupancy models [@mackenzie2003estimating; @royle2007bayesian], N-mixture models [@royle2004n], mark-recapture models [@jolly1965explicit; @calvert2009hierarchical], and other hidden Markov models [@patterson2009classifying; @langrock2012flexible]. 
...
To provide a concrete use case, a neural dynamic occupancy model is developed for extinction and colonization dynamics of North American bird communities.
...
This case study uses BBS data from 1997-2018 excluding unidentified or hybrid species, restricting the analysis to surveys which meet the official BBS criteria [@bbs2018]. 
The resulting data consists of `r printnum(summary_df$n_species)` species sampled at `r printnum(summary_df$n_routes)` routes, for a total of `r printnum(summary_df$n_surveys)` surveys (not every route is surveyed in each year), `r printnum(summary_df$n_stops)` stops, and `r printnum(summary_df$n_detection_records)` detection/non-detection observations.
...
Heterogeneity in parameter values among routes, years, species, and surveys was represented differently in three models of increasing complexity: a baseline model that maps input features to parameters with a linear combination on the logit scale [@mackenzie2003estimating], a single-species neural hierarchical model that maps inputs to parameters using a neural network with one hidden layer, and a multi-species deep neural hierarchical model (Fig. \@ref(fig:figure2)c-e). 
For all models, input features included Environmental Protection Agency (EPA) level one ecoregions, the first eight principal components of the standard 19 WorldClim Bioclimatic variables averaged across the years 1970-2000, computed for the coterminous United States [@fick2017worldclim], BBS route spatial coordinates, distance from the coast, elevation, and road density within 10 km from the Global Roads Inventory Project [@meijer2018global]. 
...
The neural hierarchical model architectures were motivated by joint species distribution models in which species load onto a shared set of latent factors [@thorson2015spatial; @warton2015so; @ovaskainen2016uncovering; @thorson2016joint; @tikhonov2017using], and by recent work on deep neural basis expansions [@mcdermott2019deep; @wikle2019comparison]. 
Analogously, the neural networks combined inputs into a latent vector for each route via a series of hidden layers, which is then passed as inputs to hidden layers associated with initial occupancy, colonization, extinction, and detection probabilities. 
These hidden layers act as latent factors, mapped via a loading matrix to process and observation parameters. 

The architecture of the multi-species neural dynamic occupancy model additionally built upon previous work on deep multi-species embedding [@chen2016deep] and encoder-decoder architectures used in sequence to sequence translation [@sutskever2014sequence].
Route-level features were encoded via a series of hidden layers, then passed to a recurrent neural network that decodes a multivariate time series of latent vectors associated with colonization, persistence, and detection [@chung2014empirical]. 
An entity embedding was constructed for each species that combined information among species within genera, families, and orders, and a neural network parameterized species-specific weights for each latent spatiotemporally referenced route feature. 
The dot product of species-specific weights and latent spatiotemporal route features generated values for colonization, persistence, and detection probabilities. 
For additional details, see Appendix S2 in Supporting Information.
...
To facilitate predictive performance comparisons, EPA level two ecoregions were randomly assigned to training, validation, and test sets with equal probability [@roberts2017cross].
...
Training routes were used for parameter estimation, and validation routes were used to select the best performing model based on validation set log-likelihood. 
The best performing model was retrained using the training and validation data, and the withheld test set routes were used to estimate predictive performance of the final model [@russell2016artificial]. 
Predictive performance was evaluated by computing 95% prediction interval coverage of test set counts, and the area under the receiver operator characteristic curve (AUC) for binarized detection data.
For the AUC analysis, cases were taken to be detections $y_{t, s, j} > 0$ and controls were non-detections $y_{t, s, j} = 0$. 
Predicted test set observation probabilities were constructed via marginalization over latent states so that $\text{Pr}(y_{t, s, j} = 0 \mid p_{t, s, j}, \psi_{t, s, j}) = 1 - \psi_{t, s, j} + \psi_{t, s, j}(1 - p_{t, s, j})^{50}$. 
Predicted occupancy probabilities for withheld test set routes were computed recursively following @mackenzie2003estimating.

```{r roc-test, fig.cap='(a) Receiver operator characteristic curves of binarized detection/non-detection data for each survey route in the test set, colored by the area under the curve (AUC). Here, the x-axis is the complement of specificity: the ratio of the number of false positives (incorrectly predicted detections, with no observed detections) to the sum of false positives and true negatives (correctly predicted non-detections with observed non-detections). The y-axis is true positive rate: the ratio of the number of true positives (correctly predicted detections with observed detections) to the sum of true positives and false negatives (incorrectly predicted non-detections with observed detections). The overall distribution of AUC values is shown in (b), and (c) shows the locations of test set routes colored by AUC values, where black dots represent routes that were used to train the final model.'}
knitr::include_graphics(here('fig', 'roc-test.jpg'))
```


Predictions from final model were qualitatively evaluated to see whether they are plausible, given previous work.
The most likely occupancy states were computed at all BBS routes for each year and species [@viterbi1967error], which were then used to compute finite sample population growth rates for each species [@royle2007bayesian].
To quantify range shifts, annual spatial centroids were computed for each species among routes that were predicted to be occupied (hereafter referred to as "BBS range centroids" to differentiate from the actual centroid of a species' entire range). 
To evaluate whether the results were consistent with previous findings that population persistence probabilities are highest in the center of a species' range, correspondence between BBS range centroids and persistence probabilities were also assessed using linear regression [@mehlman1997change; @royle2007bayesian].
These qualitative analyses only used BBS routes surveyed in every year, and species that occurred in all years and at 100 or more routes to avoid bias associated with recently added Canadian routes, and variance associated with rare species [@sauer2017expanding].

Finally, to facilitate interpretation of the final model, additional analyses were performed that provide insight into how the model represented BBS routes and bird species. 
...
In this lower dimensional space, routes with similar embeddings are close together, and routes with dissimilar embeddings appear distant.
By mapping original inputs into this lower dimensional space, it is possible to evaluate how model inputs relate to latent representations [@rauber2016visualizing]. 
Second, species' loading vectors, which described how spatiotemporal route features are weighted by process and observation parameters, were compared in terms of cosine similarity, which measures the orientation of loading vectors in latent space, with the expectation that species' occupancy will be positively related if loading vectors are oriented similarly.
...
The multi-species neural hierarchical model performed best on the withheld validation routes, with a difference in mean validation set negative log likelihood of `r round(valid_nll$ss_nll - valid_nll$nn_nll, 3)` relative to the baseline model and `r round(valid_nll$ss_nll - valid_nll$sn_nll, 3)` relative to the single-species neural hierarchical model.
...
The model also predicted whether species would be detected on test set routes fairly well, with a mean test set AUC of `r round(mean(auc_df$.estimate), 3)`, an among-route standard deviation of `r round(sd(auc_df$.estimate), 3)`, a minimum of `r round(min(auc_df$.estimate), 3)`, and a maximum of `r round(max(auc_df$.estimate), 3)` (Fig. \@ref(fig:roc-test)).


```{r centroid-displacement, fig.cap='(a) Scatter plot relating the centroid displacement of bird species (x-axis) from 1997 to 2018 in kilometers to finite-sample growth rate (y-axis), where a growth rate of 1 is stable, values less than one are decreasing, and values greater than one are increasing. Species with the highest population growth rates are highlighted. Panel (b) shows the locations of breeding bird survey range centroids in 1997 and 2018 for each highlighted species, along with blue points that represent survey routes where the species was estimated to be present in at least one year.', out.width = "400px"}
knitr::include_graphics(here('fig', 'centroid-displacement.jpg'))
```
...
The model identified the invasive Eurasian Collared-Dove (*Streptopelia decaocto*) as having the greatest range centroid displacement from 1997 to 2018 and the highest finite sample population growth rate (Fig. \@ref(fig:centroid-displacement)), consistent with its invasion of North America from Florida following its introduction in the 1980's [@bled2011hierarchical].
...
```{r persist-dist, fig.cap='(a) Persistence probabilities (y-axis) as a function of distance from range centroid, averaged across years for the species with the largest positive relationship between distance from range centroid and persistence probability (top row), and largest negative relationship (bottom row). Each point represents a North American Breeding Bird Survey route. (b) Maps of persistence probabilities at each breeding bird survey route where these species were estimated to be present (in color), with route locations where the species was never estimated to be present in grey. Centroids for each year are shown in solid black. (c) Scatter plot relating the total number of occurrences across all routes and years (x-axis) to the slope of the relationship between distance from range centroid and persistence probability ($d_c$, on the y-axis). Each point represents a species. Points below the dashed line correspond to species with lower persistence at routes distant from their range centroids.', out.width = "400px"}
knitr::include_graphics(here('fig', 'persist-dist-plot.jpg'))
```
...
Examples from the species with the greatest positive and negative relationships between centroid distance and persistence are shown in Fig. \@ref(fig:persist-dist)a-b.
Species that occurred in many routes and in many years tended to show consistently negative relationships between persistence and distance from range centroids (Fig. \@ref(fig:persist-dist)c). 

```{r route-tsne, fig.cap='A two dimensional plot of North American Breeding Bird Survey route embeddings, computed using t-distributed stochastic neighbor embedding (t-SNE) on the latent vectors for initial occupancy, persistence, colonization, and detection probabilities for each route. Color indicates (a) EPA level 1 ecoregion and (b) z-standardized continuous route-level features. PC1 refers to the first principal component of WorldClim climate data. Notice clustering both by ecoregion, and by continuous route-level features.', out.width = "400px"}
knitr::include_graphics(here('fig', 'route_tsne.jpg'))
```

Route vectors combined information from the categorical and continuous route-level features.
Unsurprisingly (because ecoregion was an input feature), routes in the same ecoregions clustered together (Fig. \@ref(fig:route-tsne)a).
Route embeddings also capture learned relationships among ecoregions. 
For example, Marine West Coast Forest ecoregion routes were similar to Northwestern Forested Mountains routes, and most different from Eastern Temperate Forests routes (Fig. \@ref(fig:route-tsne)a). 
Variation within clusters also related to continuous route-level features (Fig. \@ref(fig:route-tsne)b). 

```{r occupancy-scatter, fig.cap='Relationships among species estimated occupancy probabilities through time, by location for species pairs that are nearest neighbors in weight cosine distance (left column), and most cosine dissimilar (right column). Color indicates EPA level 1 ecoregions. Cosine similarity values for species pairs are printed in the upper corners of each panel. Each route is a line segment that connects estimates of occupancy probabilities for each year from 1997 to 2018.', out.width = "400px"}
knitr::include_graphics(here('fig', 'occupancy_scatter.jpg'))
```

The model predicted non-linear dependence among species that is interpretable in terms of the cosine similarity among species-specific weight vectors.
For example, weights for Mourning Dove (*Zenaida macroura*) were closest to those of `r cosine_sim$nearest_species[cosine_sim$english == 'Mourning Dove']` (*`r bbs_species$sciname[bbs_species$english == cosine_sim$nearest_species[cosine_sim$english == 'Mourning Dove']]`*), and most different from `r cosine_sim$farthest_species[cosine_sim$english == 'Mourning Dove']` (*`r bbs_species$sciname[bbs_species$english == cosine_sim$farthest_species[cosine_sim$english == 'Mourning Dove']]`*) (Fig. \@ref(fig:occupancy-scatter)a).
...
Species pairs of the most similar and most dissimilar loadings are also provided for Eurasian Collared-Dove and Wilson's Warbler (Fig. \@ref(fig:occupancy-scatter)b-c).
...
The analysis of breeding bird survey data indicate that neural hierarchical models can outperform simpler models. 
...
The case study extends previous analyses of persistence probabilities at range edges [@royle2007bayesian], indicating that common species tended to be more persistent at routes close to their BBS range centroid. 
The strongest exception to this trend might be a consequence of the centroid providing a poor measure of central tendency of the BBS range.
In particular, for the `r highest_slope_species_english` (*`r highest_slope_species_sciname`*), which had the strongest positive relationship between persistence probability and distance from BBS range centroid, the BBS range is irregularly shaped such that the centroid lies at the range boundary, and the northern range of the species is not as well represented in BBS routes. 
This highlights some challenges posed by irregular range geometry and incomplete sampling of species ranges for macroecological studies that have been pointed out for similar questions about species abundance at observed range edges [@sagarin2002abundant; @dallas2017species; @knouft2018appropriate].
...
Other ecological data structured on regular grids such as modeled climate data, remotely sensed Earth observations, and even 96 well microplates also might use convolutional neural network architectures. 
...
Interpretations map abstract concepts to domains that humans can make sense of, e.g., mapping latent vectors or weights of neural network to species identity or spatial location [@montavon2018methods].
...
Often, $L_2$ norm penalties (also referred to as "weight decay") are applied on the weights of neural networks in the loss function to penalize model complexity in an effort to avoid overfitting. 
...
Finally, unlike maximum likelihood estimation for simple models, because of the multimodality of the likelihood surface for neural network weights, there are no guarantees that a global maximum exists, or that a particular optimum is not a local maximum. 
...
where $W^{(1)}$ and $W^{(2)}$ are weight matrices for the first and second layer, $g$ is a differentiable nonlinear activation function, e.g., the rectified linear unit activation function [@nair2010rectified], and the inverse logit transform is applied element-wise to ensure that $\psi_i$ and $p_i$ lie between 0 and 1. 
...
where $W^{(1)}$ is a weight matrix that generates activations from the inputs, $g$ is the ReLU activation function, $W^{(2)}$ is a weight matrix that maps the hidden layer to the outputs, and $\text{logit}^{-1}$ is the element-wise inverse logistic (sigmoid) function.
...
Here too $W^{(1)}$ is a weight matrix that generates activations from the inputs, $g$ is the rectified linear unit activation function, and $W^{(2)}$ is a weight matrix that maps the hidden layer to the outputs.
...
At time $t$, the transition probability matrix $\Gamma_t$ contains the probability of transitioning from row $j$ to column $k$:
...
### Heterogeneity in occupancy dynamics {-}
...
Then, let $W^{(L_1)}$ represent a weight matrix of size $D^{(L_1)} \times R$, where $D^{(L_1)}$ is the dimensionality of the embedding ($D^{(L_1)}=8$). 
...
where $W^{(1)}$ is a $D^{(1)} \times D^{(0)}$ weight matrix.
...
Here $W^{(\psi_1)}$, $W^{(\gamma)}$, and $W^{(\phi)}$ are $4 \times 1$ weight matrices, and $W^{(p)}$ is a $(4 + n_p) \times 1$ weight matrix that maps the concatenation of detection-related hidden unit vector $h_s^{(p)}$ with $n_p$ survey specific features contained in the vector $x^{(p)}_{t, s}$ to the detection probability $p_{t, s}$.
...
Heterogeneity in occupancy dynamics was introduced by allowing species initial colonization, persistence, colonization, and detection probabilities to relate to latent spatiotemporal features (Fig. \@ref(fig:figs2)).
For the multi-species model, all embeddings are 64 dimensional, all hidden layers are 32 dimensional unless stated otherwise. 
...
where $h_{r[s]}^{(L_1)}$ is the embedding for a level one ecoregion $r$ containing route $s$, $W^{(L_1)}$ is a weight matrix, and $x_{r[s]}^{(L_1)}$ is a one-hot encoding for the level one ecoregion $r$ containing route $s$.
...
This is then passed to a sequence of hidden layers:
...
These latent spatiotemporal route features are then combined with species-specific weights generated via deep multi-species embedding [@chen2016deep].

### Hierarchical deep multi-species embedding {-}

Species-specific weights were modeled as outputs of a neural network that ingests species-level traits (in this case, species identity, genus, family, and order).
...
Here $h_j$ is the vector valued embedding for species $j$, $W^{(\text{Species})}$ is a weight matrix, $x_j$ is a one hot encoded vector, $h_j^{(g)}$ is the vector valued embedding for the genus containing species $j$, etc.
...
These hidden layers are then mapped to species-specific weights via fully connected layers with identity activation functions:
...
By modeling weights as outputs of a neural network with shared features, relationships among weights for occupancy and detection parameters can be learned. 
...
### Combining route features and species-specific weights {-}

A dot product combines latent spatiotemporal route features with species-specific weights:
...
Penalized maximum likelihood estimates of the neural hierarchical model were obtained with Pytorch [@paszke2017automatic], with L2 regularization penalties on the network weights, which is equivalent to maximum a posteriori estimation for the neural network with Gaussian priors for the weights [@blundell2015weight]. 
