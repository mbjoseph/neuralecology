...
Furthermore, neural architectures specialized to operate on structured data (e.g., convolutional neural networks for gridded data) can be readily integrated into such models (see Appendix S2 in Supporting Information for an example of a convolutional neural hierarchical model of animal movement).
...
The case study used a gated recurrent neural network architecture to capitalize on temporal structure [@chung2014empirical], and architectures designed for other data structures present additional opportunities for ecological applications. 
...
Appendix S2 provides a concrete example where gridded data (aerial imagery) are mapped to a state transition matrix of a hidden Markov model. 
...
For this reason, performance comparisons with simpler baseline models are useful, especially if those simpler models are easy to implement and understand. 
...
In large data settings, considerable computational resources are required for training retraining the same model many times might make k-fold cross validation infeasible.
...
However, the intrepretation and explanation of such models is an active area of research [@roscher2019explainable]. 
...
The convolutional hidden Markov model is a neural hierarchical model that maps an image chip $X^{(t)}$ centered on an animal's location at time $t$ to a transition probability matrix $\vec{\Gamma}^{(t)}$.
...
In addition to using dropout to regularize the convolutional model, an $L_2$ penalty of $10^{-6}$ was also applied. 
...
More broadly, this convolutional animal movement model can be thought of as just another hidden Markov model, but with raster-valued inputs. 
...
As the training set increased in size to more than 64 trajectories, the convolutional movement model's performane exceeded the point extraction baseline (Fig. \@ref(fig:conv-hmm-perf)). 
When the training set was relatively small (64 trajectories or fewer), the convolutional movement model performed worse than the point extraction baseline. 
...
But, restricting attention to models that only have access to the RGB imagery, the convolutional movmeent model was preferred. 
...
```{r transition-densities, fig.cap='Joint densities of true (x-axis) and estimated (y-axis) state transition probabilities in the final convolutional movement model. Panel (a) shows the distribution for $\\gamma_{1, 2}$ (transitions to "foraging"), with marginal histograms for the true and estimated probabilities. Panel (b) shows the same for $\\gamma_{2, 1}$ (transitions to "in transit"). '}
...
```{r top-prob-chips, fig.cap='Top eight test set image chips with the highest probabilities of transitioning into \"in transit\" from \"foraging\" (a), and from \"in transit\" to \"foraging\" (b).', out.width="450px"}
