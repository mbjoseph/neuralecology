...
Convolutional neural networks provide one method to map raster inputs to various outputs (including hierarchical model parameters).

This appendix describes simulations to evaluate the potential for convolutional neural networks (and more conventional models) to extract relevant information from spatiotemporal features for animal movement models. 
...
These canopy height models are estimated using lidar data on the NEON Airborne Observation Platform - a plane that flies over NEON sites regularly and collects a variety of data, including high resolution aerial imagery  (Fig. \@ref(fig:chm-rgb)). 
...
This high (10 cm) resolution aerial red-green-blue (RGB) imagery should to relate in some way to the 1 m resolution canopy height model, but the mapping from a 10 cm RGB image to a 1 m canopy height model is likely complex. 
This provides an opportunity to test whether a convolutional neural hierarchical model can extract relevant features from imperfect contextual inputs. 
...
The probabilities of transitioning between states is time-varying, and is summarized in a matrix $\Gamma^{(t)}$, which contains the transition probabilities $\gamma_{i, j}^{(t)}$ for states $i, j = 1, 2$ at time $t$. 
...
So for example $\gamma_{1, 2}^{(t)}$ would provide the probability of transitioning from "foraging" in time $t$ ($s_t = 2$) to "in transit" in time $t+1$ ($s_{t + 1} = 1$).
...
To ensure that "foraging" was the more likely state in the canopy, and "in transit" was the most likely state over bare ground, the true state transition probabilities in the simulation were modeled as a linear function of canopy height on a logit scale: 

$$\text{logit}(\gamma_{1, 2}^{(t)}) = -6 + 40x_t,$$
$$\text{logit}(\gamma_{2, 1}^{(t)}) = 6 - 40x_t,$$

where $x_t$ is the scaled canopy height at an animal's location in time $t$.
This fully specifies the state transition matrix, because the rows must sum to one, implying for example that $\gamma_{1, 1}^{(t)} = 1 - \gamma_{1, 2}^{(t)}$ (Fig. \@ref(fig:movement-distributions)A).

```{r movement-distributions, fig.cap='Panel A shows the true relationship between scaled canopy height (x-axis) and state transition probabilities in the simulation. Panels B and C show the densities of step sizes (B) and turn angles in polar coordinates (C) used in the simulation, colored by behavioral state.'}
...
Formally, if $z_t$ summarizes movement in the interval from time $t$ to $t+1$, it is common to consider $z_t$ as a vector containing two quantities: the step size $l_t$ and turning angle $\phi_t$, so that $z_t = (l_t, \phi_t)$ [@patterson2017statistical]. 
Thus, the movement model is a hidden Markov model with states $\{s_t\}_{t=1}^T$, transition probability matrices $\{\Gamma^{(t)}\}_{t = 1}^T$, and emissions $\{z_t\}_{t=1}^T$ (Fig. \@ref(fig:hmm-general)). 

```{r hmm-general, fig.cap='Graphical representation of the hidden Markov model for animal movement. The behavioral state $s_t$ is associated with a state transition probability matrix $\\Gamma^{(t)}$, with observations $z_t$ that represent the movement trajectory of the animal.', out.width = "250px"}
...
\end{cases},$$
for $t=1, ..., T$, where the value used at $t=1$ corresponded to the initial direction of travel in the simulation (rather than a turn angle strictly defined, which requires two direction vectors to compute). 
...
1. A **best case model** that takes canopy height as a state transition covariate. This represents the perfect scenario (unlikely in practice) where all relevant spatiotemporal information is provided to the model. 
2. A **point extraction model** that takes the red, green, and blue reflectance values from the aerial imagery extracted at point locations. This represents a more common scenario where covariate data indirectly related to the relevant spatiotemporal information (canopy height) are extracted at point locations. 
3. A **convolutional neural hierarchical model** that takes image chips centered on point locations as input, and maps these image chips to transition matrices using a convoluitonal neural network. If RGB image chips contain more information about canopy height than simple RGB point extractions, this model should perform better than the point extraction model. 
