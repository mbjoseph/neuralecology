...
The convolutional neural hierarchical maps an image chip $X^{(t)}$ centered on an animal's location at time $t$ to a transition probability matrix $\Gamma^{(t)}$.
...
To generate image chip input arrays, square crops from the aerial RGB imagery were created centered on each simulated location so that the spatial footprint of each chip was 128 $\times$ 128 pixels. 
...
To illustrate the potential for including additional raster data in addition to imagery, an additional band was concatenated to each chip which contained zeros everywhere except for a $2 \times 2$ region in the center of the $128 \times 128$ grid (in real applications, this might represent additional raster layers that could be relevant for movement).

The convolutional hidden Markov model for animal movement mapped these $4 \times 128 \times 128$ image chips to $2 \times 2$ state transition probability matrices $\{ \Gamma^{(t)} \}_{t = 1}^T$ (Fig. \@ref(fig:traj-plot)). 
...
Essentially, the input image is passed through a series of 2-dimensional convolutions, followed by nonlinear activation functions, followed by 2d max-pooling layers, creating a $64 \times 2 \times 2$ lower spatial resolution array with many "channels". 
This array is "flattened" to create a vector of length $64 \times 2 \times 2$, which is then passed to a series of fully connected hidden layers to create a vector of length 4. 
