...
First, single-species baseline models mapped input features to occupancy parameters with a linear combination on the logit scale [@mackenzie2003estimating]. 
Second, single-species neural hierarchical models mapped inputs to occupancy parameters using a neural network with one hidden layer.
Third, a multi-species deep neural hierarchical model was developed to model occupancy dynamics of all species simultaneously (Fig. \@ref(`r ifelse(params$preprint, "fig:figure2", "fig:endfig1")`)c-e). 
...
Predictions from final model were qualitatively evaluated to see whether they are plausible, given previous analyses of BBS data.
...
These predicted occupancy states were also used to compute annual spatial centroids for each species, using the coordinates of routes that were predicted to be occupied.
...
For this qualitative analysis, the response variable was colonization or persistence, and the predictor was distance from BBS range centroid. 
...
# Appendix S1: example model specifications {-}
...
# Appendix S2: a convolutional animal movement model {-}
...
This appendix includes a description of embedding a convolutional neural network in a neural hierarchical model. 
...
In an ecological context, CNNs have been used to identify plants and animals, e.g., from aerial imagery and camera traps [@norouzzadeh2018automatically; @fricker2019convolutional; @tabak2019machine]. 
However, CNNs considered more broadly as function approximators have additional uses. 
For example, instead of mapping an image to a vector of class probabilities, a CNN might map an image to a state transition probability matrix in a hidden Markov model [@zucchini2017hidden]. 

To demonstrate the usage of CNN architectures in neural hierarchical models, this appendix describes a neural animal movement model and demonstrates its use in a semi-realistic simulation.
Here, a CNN is used to model state transition probabilities of a hidden Markov model.
Generally, the movmement of an individual animal depends on the spatial and temporal context around their location at any time, and on their behavioral state in that location at that time. 
For example, for a two-state animal movement model, the behavioral state of an individual at time t $s_t$ might be one of "foraging" ($s_t = 1$) or "in transit" ($s_t = 2$). 
Let the probability of transitioning among states be time-varying so that the time series of states is a non-homogeneous Markov chain, and the time varying transition transition probability matrix $\Gamma^P{t} = (\gamma_{i, j}^{(t)})$ is a $2 \times 2$ square matrix where $\gamma_{i, j}^{(t)}$ is the probability of transitioning from $s_t = i$ to $s_{t+1} = j$.

In many animal movement models, covariates for the state transitions are included using a linear combination of values followed by a softmax transformation to ensure that probabilities within rows sum to one. 
This allows inference about spatiotemporal covariates that affect behavior and thus movement, such as distance to water, temperature, hour of day, wind, and ocean surface current [@patterson2017statistical; @johnson2018continuous; @mcclintock2018momentuhmm]
All of these examples have used a relatively simple approach for extracting covariate features from trajectory data: namely by using the values of the covariates at the spatiotemporal point locations of an individual's movement path. 
As a consequence, the models learn to map covariate vectors (at point locations) to transition probability matrices. 
However, it is reasonable to expect that the spatiotemporal context around these point locations could also be relevant. 

Understanding how exactly to include such context is not immediate, but convolutional neural networks provide one option. 
Instead of extracting values at points, consider using raster data at the point location and around the point location (e.g., image "chips" centered around observed point locations, including all data within some spatiotemporal region).
Inputs might then be rasters instead of vectors, containing imagery (satellite and/or aerial), landscape features (e.g., a digital elevation model or land cover map), or meteorological data (e.g., gridded temperature or wind speed data).

```{r conv-hmm, fig.cap='A convolutional neural network that maps a raster (in this case a 4 $\\times$ 128 $\\times$ 128 grid) to a state transition probability matrix of a hidden Markov model. Yellow boxes indicate input arrays and outputs from convolutional layers, with labeled dimensions. Red boxes represent two dimensional maximum pooling layers. Purple boxes represent fully connected hidden layers. The final vector of length 4 is reshaped to a 2 by 2 matrix, and then a softmax transform is applied row-wise to ensure that the rows sum to one for the transition probability matrix at timestep $t$, denoted $\\Gamma^{(t)}$.'}
knitr::include_graphics(here('fig', 'conv_hmm_edited.pdf'))
```

Consider the following scenario. 
Animals move over a landscape and their positions are sampled at regular time intervals. 
These animals prefer to forage in and around tree canopies, but they frequently move between canopies over bare ground. 
To generate a semi-realistic simulation of this scenario, movement trajectories were simulated over real canopy height models from the San Joaquin Experimental Reserve, one of the core National Ecological Observatory Network (NEON) sites [@keller2008continental]. 
Canopy height models are estimated using lidar data on the NEON Airborne Observation Platform - a plane that flies over NEON sites regularly and collects a variety of data, including high resolution aerial imagery. 

This simulation addresses a fundamental challenge in movement modeling, namely: how might spatiotemporal context inform behavioral state transitions?
If the "correct" spatiotemporal covariates are known and available (e.g., canopy height), and the relevant information is entirely contained in the point location of an individual at any given time, inference about state transitions is relatively easy. 
However, this is rarely the case. 
When relevant contextual covariates are unknown or unobserved, inference is more difficult. 

Sometimes, covariate data are indirectly related to state transitions, but the functional mapping from these ancillary data to the "correct" spatiotemporal covariates may be complex and unknown *a priori*. 
For example, at NEON, high (10 cm) resolution aerial red-green-blue (RGB) imagery is collected by the AOP, which we expect to relate in some way to the 1 m resolution canopy height model.

Hypothetically, if canopy height is the covariate that really matters for movement but it was unobserved, how useful is RGB imagery?
The mapping from a 10 cm RGB image to a 1 m canopy height model is likely complex. 
Indeed, it is unclear whether RGB imagery would contain sufficient information to reconstruct a canopy height model, or quantities that are functions of canopy height (such as state transition probabilities).
As a human observer looking at an RGB image, it is not obvious what the height of the canopy might be (in contrast to a more traditional image classification task that labels images of cats, dogs, elephants, etc.).
This provides an opportunity to test whether a convolutional neural hierarchical model can extract relevant features from imperfect inputs to infer state transitions in an ecological model. 

```{r chm-rgb, fig.cap='An example of a 1 meter resolution canopy height model (left) and corresponding 10 cm aerial RGB image over the San Joaquin Experimental Reserve. In the simulation, canopy height affects behavioral state transitions and subsequent animal movement, but the available data might consist only of the RGB imagery. A 250 m by 250 m subset of the study area is displayed.'}
knitr::include_graphics(here('fig', 'chm-rgb'))
```

## Simulating animal movement through tree canopies {-}

To evaluate the potential for convolutional neural networks in a semi-realistic setting, individual animal movement trajectories were simulated over a real canopy height model of the San Joaquin Experimental Reserve. 
...
Due to some extreme observations (> 80m) in the canopy height model, which likely represent errors, all values greater than 30m (~0.0025% of cells) were set to 30m. 
...
The assumption was that animals were more likely to forage where the canopy height is high, and more likely to be in transit where canopy height is low. 
This was enforced in the simulation by modeling state transition probabilities as a function of canopy height. 

- Distributions of step size and turn angles

- Random initial location

- Pruning of trajectories that fall outsize the study area
...
These neural networks have also been combined with hidden Markov models in speech recognition contexts


# Appendix S3: breeding bird survey model details {-}
