...
the function $f$ maps covariates $X$ to occupancy probability $\psi$:
...
function $f$ is parameterized by $\theta$:
...
- Occupancy models (nondetection $\neq$ absence) 
- N-mixture models (count $\neq$ abundance)
- Capture-recapture models (not captured $\neq$ dead)
- Animal movement models (location $\neq$ behavioral state)
