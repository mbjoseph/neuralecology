...
def pt_Omega(rgb_pt, coefs):
  ...
  t1to2 = torch.sigmoid(coefs[0, 0] + torch.sum(rgb_pt * coefs[0, 1:].view(1, 1, -1), -1))
  t2to1 = torch.sigmoid(coefs[1, 0] + torch.sum(rgb_pt * coefs[1, 1:].view(1, 1, -1), -1))
  t1to1 = 1 - t1to2
  t2to2 = 1 - t2to1
...
for n in train_data_sizes:
  ...
  bestcase_gamma_pars = torch.tensor(bestcase_wt[["gamma_shape", "gamma_rate"]].values, dtype=torch.float)
  bestcase_loc_pars = torch.tensor(np.squeeze(bestcase_wt[["vm_mean"]].values), dtype=torch.float)
  bestcase_conc_pars = torch.tensor(np.squeeze(bestcase_wt[["vm_concentration"]].values), dtype=torch.float)
  bestcase_coefs = torch.tensor(bestcase_wt[["transition_intercept", "z"]].values, dtype=torch.float)

  # baseline pt model
  pt_wt = pd.read_csv(f"../out/params/ptextract_{n}.csv")
  pt_gamma_pars = torch.tensor(pt_wt[["gamma_shape", "gamma_rate"]].values, dtype=torch.float)
  pt_loc_pars = torch.tensor(np.squeeze(pt_wt[["vm_mean"]].values), dtype=torch.float)
  pt_conc_pars = torch.tensor(np.squeeze(pt_wt[["vm_concentration"]].values), dtype=torch.float)
  pt_coefs = torch.tensor(pt_wt[["transition_intercept", "rgb_mosaic.1", "rgb_mosaic.2", "rgb_mosaic.3"]].values, dtype=torch.float)
  ...
  for i_batch, (xy, idx) in enumerate(tqdm(loaders["valid"])):
    ...
    bestcase_loglik = simutils.forward_algorithm(
      step_size, turn_angle, bestcase_Omega(xy["chm"], bestcase_coefs), 
      bestcase_gamma_pars, bestcase_loc_pars, bestcase_conc_pars
    )
    bestcase_loss.append(float(-torch.sum(bestcase_loglik.detach())))
    pt_loglik = simutils.forward_algorithm(
      step_size, turn_angle, pt_Omega(xy["rgb_pt"], pt_coefs), 
      pt_gamma_pars, pt_loc_pars, pt_conc_pars
    )
